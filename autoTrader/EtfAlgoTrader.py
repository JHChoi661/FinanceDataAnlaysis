...
slack = Slacker('xoxb-1600521518790-1613463840660-b1Y1jLadaVANuWUJpGYTcBOL')
def dbgout(message):
...
def get_hoding_stocks():
    """sell_all에서 제외할 종목들을 구한다.(홀딩하고 있는 종목)"""
    ...
    for i in range(cpBalance.GetHeaderValue(7)):
        code.append(cpBalance.GetDataValue(12, i))  # 종목코드
        name.append(cpBalance.GetDataValue(0, i))   # 종목명
    return code, name
...
def sell_all():
    ...
    try:
        ...
        while True:    
            ...
            stocks = [x for x in stocks if x['code'] not in holding_list] # stocks에서 holding list에 있는 종목들 제거
            total_qty = 0 
            for s in stocks:
                total_qty += s['qty'] 
            if total_qty == 0:
...
""" def test():
    ...
    accFlag = cpTradeUtil.GoodsList(acc, 1)  # -1:전체, 1:주식, 2:선물/옵션   
    stocks = get_stock_balance('ALL') 
    stocks = [x for x in stocks if x['code'] not in holding_list]
    print(stocks)
    total_qty = 0 
    for s in stocks:
        total_qty += s['qty'] 
...
if __name__ == '__main__': 
    try:
        ...
        holding_list, holding_name = get_hoding_stocks()
        # test()
        stocks = get_stock_balance('ALL')      # 보유한 모든 종목 조회
        total_cash = int(get_current_cash())   # 100% 증거금 주문 가능 금액 조회
        buy_amount = total_cash * buy_percent  # 종목별 주문 금액 계산
        printlog('매매 제외 종목 :', holding_name)
